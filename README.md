...
FORWARDED_ALLOW_IPS: Optional. Controls which IP addresses are trusted to provide forwarded headers (X-Forwarded-For, X-Forwarded-Proto, etc.) when MediaFlow Proxy is deployed behind reverse proxies or load balancers. Default: 127.0.0.1. See Forwarded Headers Configuration for detailed usage.
...
Simple proxy setup with SSL bypass for internal domain:
...
Best Practices
Principle of Least Privilege: Only trust the specific IP addresses of your proxy servers
Regular Updates: Keep your trusted IP list updated when infrastructure changes
Monitor Logs: Watch for unexpected forwarded headers from untrusted sources
Test Configuration: Verify that client IPs are correctly detected after configuration changes
Troubleshooting
...
Check the logs in Nginx Proxy Manager for 403 errors
Test problematic URLs directly
Consider disabling "Block Common Exploits" if it interferes with streaming
Implement alternative security measures (Cloudflare WAF, fail2ban, etc.)
...
Open your browser and navigate to http://your-server:8888/speedtest.html
The speed test page allows you to:
...
Visit https://store.elfhosted.com/product/mediaflow-proxy
Follow ElfHosted's setup instructions
Benefits:
...
Wait until the space gets built and deployed. Don't panic if you see "Your app is running" instead of the usual mediaflowproxy page. You can still use it as usual.
...
Usage
Endpoints
/proxy/hls/manifest.m3u8: Proxify HLS streams
/proxy/stream: Proxy generic http video streams
/proxy/mpd/manifest.m3u8: Process MPD manifests
/proxy/mpd/playlist.m3u8: Generate HLS playlists from MPD
/proxy/mpd/segment.mp4: Process and decrypt media segments
/proxy/ip: Get the public IP address of the MediaFlow Proxy server
/extractor/video?host=: Extract direct video stream URLs from supported hosts (see supported hosts in API docs)
/playlist/builder: Build and customize playlists from multiple sources
...
Character Set Check: Verifies the string contains only valid base64 characters (A-Z, a-z, 0-9, +, /, =)
Protocol Check: Ensures the string doesn't start with common URL protocols (http://, https://, etc.)
Length Check: Validates minimum length for meaningful base64 encoded URLs
Decoding Validation: Attempts to decode and validates the result is a valid URL
...
mp4decrypt, mp4box, and devine for insights on parsing MPD and decrypting Clear Key DRM protected content.
